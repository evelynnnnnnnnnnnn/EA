...
if (!empty($doctor_name)) {
    ...
    if (is_array($doctor)) { //выводим одного доктора
        ...
        $spec_names = implode(', ', array_map(function ($elem) {
            return mb_strtolower($elem["NAME"]);
        }, $specs));

        if($doctor['SPEC_IDS_MULTI']){
            $specs = SpecsTable::query()
...
if ($action == 'new' || $action == 'edit') { // добавляем доктора
    if (isset($_POST['doctor-submit'])) {
        ...
        if ($action == 'edit' && !empty($_POST['ID'])) {
            ...
            $specs = $_POST['SPEC_IDS_MULTI'];
            unset($_POST['SPEC_IDS_MULTI']);
            $procs = $_POST['PROC_IDS_MULTI'];
            unset($_POST['PROC_IDS_MULTI']);

            CIBlockElement::SetPropertyValues($ID, DoctorsTable::IBLOCK_ID, $specs, "SPEC_IDS_MULTI");
    ...
    $spec_options = SpecsTable::query()->setSelect(["ID"=>"ELEMENT.ID","NAME"=>"ELEMENT.NAME"])->fetchAll();
...
<section class="doctors">
    ...
    <?php if (is_array($doctor) && sizeof($doctor)>0 && $action!='edit'):?>
        <div class="doctor-page">
            ...
            <h3>Специализаци<?=(sizeof($specs)>1)? 'и':'я'?>: <?=$spec_names?></h3>
            <h3>Процедуры:</h3>
            <ul>
    ...
    <?php if (empty($action)):?>
        <div class="add-buttons">
            <?php if (!empty($doctor_name)):?>
                <a href="/doctors/html.php/<?=$doctor_name?>"><button>Вернуться назад</button></a>
            <?php endif;?>
        </div>
    <?php endif;?>
        ...
        <form method="POST">
            ...
            <div class="doctor-add-form">
                ...
                <select multiple name="SPEC_IDS_MULTI[]">
                    ...
                    <?php foreach ($spec_options as $spec):?>
                        <option value="<?=$spec['ID']?>"
                                <?php if (isset($data['SPEC_IDS_MULTI']) && in_array($spec['ID'],$data['SPEC_IDS_MULTI'])):?>selected<?php endif;?>>
                            <?=$spec['NAME']?>
                        </option>
                    <?php endforeach; ?>
                </select>
